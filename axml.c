...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  /* most of the stuff below is just executed by the MPI master process 
     the MPI master process steers the search and the computations and is the only process that holds 
  ...
  */ 
  if(processID == 0)
...
#else      
#ifdef _FINE_GRAIN_MPI
      /* branch off into the MPI implementation, let's not worry about this now */
...
#ifdef _FINE_GRAIN_MPI
      /* that's the last parallel region of the MPI code, we just need to make sure 
	 that all worker processes invoke MPI_Finalize() such that the MPI code 
    ...
    {
      /* actually that's the invocation of the MPI worker processes, all the code above is just invoked by the master process with rank 0 
       */
      
      /* initialize hash table stuff */
      compute_bits_in_16bits();

      /* branch off to the fine-grain worker function, basically the workers execute and endless loop and wait fpr broadcasts what they should to from the 
	 master. Once the master tells the workers to exit gracefully, they wil jump out of the endless loop and also invoke MPI_Finalize() */
...
#ifdef _FINE_GRAIN_MPI
  /* the MPI master calls MPI finalize */
  MPI_Finalize();    
#endif
