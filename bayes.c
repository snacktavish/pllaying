...
/*
static void showTreeNodes(tree *tr)
{
  int i;
  nodeptr p, q;
  ...
  for(i = tr->mxtips + 1; i <= tr->mxtips + tr->mxtips - 2; i++)
  {
    ...
    while(!p->x)
      p = p->next;
    printBothOpen("node %d b %d, nb %d nnb %d\n",  
        p->number, 
        p->back->number,  
        p->next->back->number,  
...
static boolean simpleNodeProposal(state * instate)
{
  ...
  if(instate->q!=NULL)
  {
    ...
    printBothOpen("inserted %db%d at %db%d where bl %f, Thorough is %d \n", 
                  ...
                  */
    if (! insertBIG(instate->tr, instate->p, instate->q, instate->tr->numBranches)) assert(FALSE);
...
static void simpleModelProposal(state * instate)
{
  ...
  //recalculate eigens
  initReversibleGTR(instate->tr, instate->adef, instate->model); /* 1. recomputes Eigenvectors, Eigenvalues etc. for Q decomp. */
...
void mcmc(tree *tr, analdef *adef)
{
  ...
  int maxradius = 30;
  int accepted_spr = 0, accepted_bl = 0, accepted_model = 0, accepted_gamma = 0, inserts = 0;
  int rejected_spr = 0, rejected_bl = 0, rejected_model = 0, rejected_gamma = 0;
  int num_moves = 10000;
  ...
  for(j=0; j<num_moves; j++)
  {
   ...
   //printBothOpen("proposal done, iter %d tr LH %f, startLH %f\n", j, tr->likelihood, tr->startLH);
    assert(which_proposal == SPR || which_proposal == UPDATE_ALL_BL || which_proposal == UPDATE_MODEL
      || which_proposal == UPDATE_GAMMA);
    ...
    */
	...
	//      printBothOpen("BL new , iter %d tr LH %f, startLH %f\n", j, tr->likelihood, tr->startLH);
      ...
      else if(which_proposal == UPDATE_MODEL)
      {
//	printBothOpen("Model new, iter %d tr LH %f, startLH %f\n", j, tr->likelihood, tr->startLH);
      }
      else if(which_proposal == UPDATE_GAMMA)
      {
//	printBothOpen("Gamma new, iter %d tr LH %f, startLH %f\n", j, tr->likelihood, tr->startLH);
	...
	rejected_gamma++;
      evaluateGeneric(tr, tr->start, FALSE); // just for validation 
  ...
  treeEvaluate(tr, 1);
  blTime += gettime() - t;
  printBothOpen("accepted SPR %d, accepted BL %d, accepted model %d, accepted gamma %d, num moves tried %d, SPRs with max radius %d\n", 
		accepted_spr, accepted_bl, accepted_model, accepted_gamma, num_moves, maxradius);
  printBothOpen("rejected SPR %d, rejected BL %d, rejected model %d, rejected gamma %d\n",
		rejected_spr, rejected_bl, rejected_model, rejected_gamma);
  printBothOpen("ratio SPR %f, ratio BL %f, ratio model %f, ratio gamma %f\n",
		accepted_spr/(double)(rejected_spr+accepted_spr), accepted_bl/(double)(rejected_bl+accepted_bl), 
