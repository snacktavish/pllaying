...
void makeP(double z1, double z2, double *rptr, double *EI, double *EIGN, int numberOfCategories, double *left, double *right, boolean saveMem, int maxCat, const int states) {
    ...
    double *lz1 = (double*) malloc(sizeof(double) * states), *lz2 =
            (double*) malloc(sizeof(double) * states), *d1 =
            (double*) malloc(sizeof(double) * states), *d2 =
            (double*) malloc(sizeof(double) * states);
...
void reorder_tip_block(uint8_t *tip_idx, double *tip_vec, size_t block_start, size_t entry_width, oiter ostart, size_t vw) {
...
template<typename oiter>
void reorder(double *v, size_t width, size_t entry_width, oiter ostart, size_t vw) {
...
void reorder_back(double *v, size_t width, size_t states, iiter ostart, size_t vw) {
...
template<typename state_t, size_t VW, size_t states>
void newviewGAMMA_FLEX(int tipCase, double *x1, double *x2, double *x3, double *extEV, double *tipVector, int *ex3, unsigned char *tipX1, unsigned char *tipX2, int n, double *left, double *right, int *wgt, int *scalerIncrement, const int maxStateValue) {
    typedef vector_unit<state_t,VW> vu;
    typedef typename vu::vec_t vec_t;


    double *uX1, *uX2, *v, x1px2, *vl, *vr, al, ar;
...
//                *umpX2 = (double*) malloc(sizeof(double) * precomputeLength);
        ...
        for (i = 0; i < n; i+=VW) {
            ...
            reorder_tip_block(tipX2, arr.umpX2.data(), i, span, arr.u_x2.begin(), VW );
...
//                }
                ...
                for (k = 0; k < states; k++) {
                    ...
                    vec_t x1px2_v = vu::mul( vu::load(arr.u_x1((j * states + k) * VW)),
...
//                    }
                    ...
                    // unrolled l-loop
                    v_acc0 = vu::add( v_acc0, vu::mul( x1px2_v, vu::set1(extEV[states * k + 0])));
                    v_acc1 = vu::add( v_acc1, vu::mul( x1px2_v, vu::set1(extEV[states * k + 1])));
                    v_acc2 = vu::add( v_acc2, vu::mul( x1px2_v, vu::set1(extEV[states * k + 2])));
                    v_acc3 = vu::add( v_acc3, vu::mul( x1px2_v, vu::set1(extEV[states * k + 3])));
...
//
//                }
                vu::store( v_acc0, arr.x3_ro(i * span + (j * states + 0) * VW));
                vu::store( v_acc1, arr.x3_ro(i * span + (j * states + 1) * VW));
                vu::store( v_acc2, arr.x3_ro(i * span + (j * states + 2) * VW));
                vu::store( v_acc3, arr.x3_ro(i * span + (j * states + 3) * VW));
        ...
        for (i = 0; i < n; i++) {
            ...
            for (k = 0; k < 4; k++) {
                ...
                for (l = 0; l < states; l++) {
                    ...
                    for (j = 0; j < states; j++)
                        ump_x2[l] += v[j]
                                * right[k * statesSquare + l * states + j];
                }
...
//                    ar = 0.0;
                    ...
                    for (j = 0; j < states; j++) {
                        al_v = vu::add( al_v, vu::mul( vu::load( arr.x1_ro(span * i + (states * k + j) * VW )),
                                                   vu::set1(left[k * statesSquare + l * states + j]) ));


                        ar_v = vu::add( ar_v, vu::mul( vu::load( arr.x2_ro(span * i + (states * k + j) * VW )),
...
//                    x1px2 = al * ar;
                    ...
                    v_acc0 = vu::add( v_acc0, vu::mul( x1px2_v,
                                                        vu::set1(extEV[states * l + 0]) ));
                    v_acc1 = vu::add( v_acc1, vu::mul( x1px2_v,
                                                        vu::set1(extEV[states * l + 1]) ));
                    v_acc2 = vu::add( v_acc2, vu::mul( x1px2_v,
                                                        vu::set1(extEV[states * l + 2]) ));
...
extern "C" {
void newviewCAT_FLEX_reorder(int tipCase, double *extEV, int *cptr, double *x1, double *x2, double *x3, double *tipVector, int *ex3, unsigned char *tipX1, unsigned char *tipX2, int n, double *left, double *right, int *wgt, int *scalerIncrement, const int states);
...
void newviewGAMMA_FLEX_reorder(int tipCase, double *x1, double *x2, double *x3, double *extEV, double *tipVector, int *ex3, unsigned char *tipX1, unsigned char *tipX2, int n, double *left, double *right, int *wgt, int *scalerIncrement, const int states, const int maxStateValue) {
    assert( states == 4 );
    sim_reorder::newviewGAMMA_FLEX<double,2,4>( tipCase, x1, x2, x3, extEV, tipVector, ex3, tipX1, tipX2, n, left, right, wgt, scalerIncrement, maxStateValue);
}
