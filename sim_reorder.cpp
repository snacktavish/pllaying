...
void reorder_back(double *v, size_t width, size_t states, iiter ostart,
		size_t vw) {
...
//                    ar = 0.0;
					...
					for (j = 0; j < states; j++) {
						...
						ar_v = vu::add(
								ar_v,
								vu::mul(
										vu::load( arr.x2_ro(span * i + (states * k + j)* VW)),
...
//                std::cout << "scale sim\n";
				...
				for (k = 0; k < 4; ++k) {
					double * __restrict base = arr.x3_ro(
...
void newviewGAMMA_FLEX_reorder(int tipCase, double *x1, double *x2, double *x3,
		...
		int *scalerIncrement, const int states, const int maxStateValue) {
	assert( states == 4);
	sim_reorder::newviewGAMMA_FLEX<double, 4, 4>(tipCase, x1, x2, x3, extEV,
			tipVector, ex3, tipX1, tipX2, n, left, right, wgt, scalerIncrement,
			maxStateValue);
}
